...
class Esercizio
{
    ...
    public function __construct(
        ...
        // core.php imposta la timezone nativa OSM.
    ...
    public static function classifyExisting(array $existing, array $expectedEntries): array
    {
        ...
        if ((int) ($existing['rows'] ?? 0) !== count($expectedEntries)) {
            return [
                'status' => 'anomaly',
                'messages' => [
                    tr('Il numero delle righe tecniche non coincide con la scrittura attesa.'),
                ],
            ];
        }
    ...
    public function execute(string $operation): int
    {
        ...
        return $connection->transaction(function () use ($operation, $database) {
            ...
            // Tutti i controlli, compreso quello temporale, vengono ripetuti al POST.
            ...
            $mastrino = Mastrino::build($description, $preview['date'], false, true);

            foreach ($preview['entries'] as $entry) {
                $movement = Movimento::build($mastrino, $entry['id_conto']);
                if ($entry['totale'] >= 0) {
    ...
    protected function getBlockingMessage(array $preview): string
    {
        ...
        if (!empty($preview['blocking_reasons'])) {
            return implode(' ', $preview['blocking_reasons']);
        }

        return tr('Operazione non disponibile per il periodo selezionato.');
    }

    /**
     * Serializza Apertura e Chiusura sui record di configurazione condivisi.
    ...
    protected function getAccountInfo(int $id): ?array
    {
        ...
        $row = database()->fetchOne(
            'SELECT c3.id,
                    c3.descrizione,
                    c1.descrizione AS gruppo,
                    CONCAT(c2.numero, ".", c3.numero, " ", c3.descrizione) AS descrizione_completa
             FROM co_piano_dei_conti3 c3
             INNER JOIN co_piano_dei_conti2 c2 ON c2.id=c3.id_piano_dei_conti2
             INNER JOIN co_piano_dei_conti1 c1 ON c1.id=c2.id_piano_dei_conti1
             WHERE c3.id=:id',
            ['id' => $id]
        );
    ...
    protected function getExisting(string $operation, string $expectedDate): array
    {
        ...
        $flaggedRows = database()->fetchArray(
            'SELECT id, id_mastrino, id_conto, data, totale, is_apertura, is_chiusura,
                    id_documento, id_scadenza
             FROM co_movimenti
             WHERE '.$flag.'=1 AND DATE(data) BETWEEN :start AND :end
             ORDER BY id_mastrino, id',
        ...
        $mastrini = array_values(array_unique(array_map('intval', array_column($flaggedRows, 'id_mastrino'))));
        $ids = implode(',', $mastrini);
        $allRows = database()->fetchArray(
            'SELECT id, id_mastrino, id_conto, data, totale, is_apertura, is_chiusura,
                    id_documento, id_scadenza
             FROM co_movimenti
             WHERE id_mastrino IN ('.$ids.')
             ORDER BY id_mastrino, id'
        );
        ...
        foreach ($allRows as $row) {
            ...
            $idConto = (int) $row['id_conto'];
            $accountTotals[$idConto] = ($accountTotals[$idConto] ?? 0.0) + (float) $row['totale'];
            $dates[substr((string) $row['data'], 0, 10)] = true;
            $flagsValid = $flagsValid
                && (int) $row[$flag] === 1
                && (int) $row[$oppositeFlag] === 0;
            $documentsLinked = $documentsLinked || !empty($row['id_documento']);
    ...
    protected function getOpeningBalances(int $closingAccount): array
    {
        ...
        return database()->fetchArray(
            'SELECT c3.id, CONCAT(c2.numero, ".", c3.numero, " ", c3.descrizione) AS descrizione, SUM(m.totale) AS totale
             FROM co_piano_dei_conti3 c3
             INNER JOIN co_piano_dei_conti2 c2 ON c3.id_piano_dei_conti2=c2.id
             INNER JOIN co_piano_dei_conti1 c1 ON c2.id_piano_dei_conti1=c1.id
             INNER JOIN co_movimenti m ON c3.id=m.id_conto

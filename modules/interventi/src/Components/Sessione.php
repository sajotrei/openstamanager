...
class Sessione extends Model
{
    ...
    public function getAliquotaAttribute()
    {
        if (!isset($this->aliquota_iva)) {
            $id_iva = $this->intervento->anagrafica->id_iva_vendite ?: setting('Iva predefinita');
    ...
    protected function getTariffa($id_tipo)
    {
        ...
        if (!empty($id_contratto)) {
            $tariffa_contratto = $database->fetchOne('SELECT costo_ore, costo_km, costo_diritto_chiamata FROM co_contratti_tipi_intervento WHERE id_contratto = '.prepare($id_contratto).' AND id_tipo_intervento = '.prepare($id_tipo));
